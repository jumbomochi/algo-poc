...
- [x] **Filing-lagged fundamentals** — key availability off filing date, not fiscal period-end (this also fixes a **live** leak in paper trading). `fetch_fundamentals.py:82-88,118-119`, `run_paper.py:652`
- [x] **Purge/embargo the holding period** in the ML walk-forward; never apply a model in-sample. `train_signal_model.py:37-114,117-141`
- [x] **Cost realism** — per-order commission floor + per-instrument slippage; use `ddof=1` Sharpe. `run_backtest.py:1948-1951`, `metrics.py:78-89`
- [x] **Rebaseline the divergence monitor** against a same-execution-model backtest (next-open fills, real costs). `backtest/divergence.py:228-302`
