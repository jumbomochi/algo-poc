...
| # | Finding | Decision |
|---|---------|----------|
| D8 | Rung-0 economics (~$34–119 positions, 84–293bps drag) | INVESTIGATED (KAN-34) — the split does not trade at all at Rung 0, not merely expensively. DECIDED 2026-08-17: option (c), Rung 0 runs `momentum` alone at 100%; five sleeves suspended-not-retired; conditional on the D10 verdict of record, fallback `sector_rotation`. **HELD 2026-08-28: the condition failed.** `momentum` scored DSR 0.358 and the fallback `sector_rotation` 0.401, against a 0.95 threshold — all six sleeves FAIL (D10). The allocation is held, not amended and not retired; **Rung 0 does not arm and epoch v2 (KAN-33) does not start.** See [rung0-economics.md §9.8](../operations/rung0-economics.md) |
| D9 | Incumbent sleeves bypass the pipeline | Grandfather via epoch v2 + retroactive kill criteria + pro-rata funding rule |
| D10 | Gate proves fidelity, not edge | Formal edge-validation framework (holdout, deflated Sharpe, parameter stability); incumbents evaluated before Rung 0. **VERDICTS OF RECORD 2026-08-28 (KAN-55): all six incumbent sleeves FAIL**, DSR 0.000–0.473 against 0.95. The single-use `incumbent_sleeves_2026` holdout is spent. Recommended stage `shadow` for each; nothing moves automatically (D3.3). This is what holds D8 — see [incumbent-edge-evaluation.md](../operations/incumbent-edge-evaluation.md) |
...
re-run, now available at Rung-0 capital and sizing. It is **not** the edge-evidence baseline
