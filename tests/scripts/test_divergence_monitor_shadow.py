...
def test_per_sleeve_series_round_trip(tmp_path) -> None:
    ...
    per_sleeve, _ = load_shadow_equity_series(load_shadow(path))
...
def test_the_aggregate_is_the_sum_across_sleeves(tmp_path) -> None:
    ...
    _, aggregate = load_shadow_equity_series(load_shadow(path))
...
def test_no_aggregate_when_there_are_no_sleeves(tmp_path) -> None:
    ...
    per_sleeve, aggregate = load_shadow_equity_series(load_shadow(path))
...
def test_a_missing_shadow_raises(tmp_path) -> None:
    ...
    Asserted on ``load_shadow``, which is where the read now happens: the
    series builder takes an already-loaded artifact so one read backs both the
    verdict and the id it is filed under. The monitor's mapping of this to
    exit 3 rather than a phantom BREACH is pinned end to end in
    ``test_divergence_monitor_shadow_e2e.py``.
    """
